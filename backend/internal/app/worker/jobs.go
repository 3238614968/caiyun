...
type workerJobLeaseStore interface {
	SetNX(key string, value interface{}, expiration time.Duration) (bool, error)
	DelIfValue(key, value string) (bool, error)
}

func runWithWorkerJobLease(lockStore workerJobLeaseStore, jobName string, ttl time.Duration, job func() error) error {
	if job == nil {
		return nil
	}
	if lockStore == nil || ttl <= 0 {
		return job()
	}
	hostname, _ := os.Hostname()
	owner := fmt.Sprintf("%s:%d:%d", hostname, os.Getpid(), time.Now().UnixNano())
	key := fmt.Sprintf("worker:cron:lock:%s", jobName)
	locked, err := lockStore.SetNX(key, owner, ttl)
	if err != nil {
		log.Printf("【Worker】获取定时任务分布式锁失败，跳过本轮 job=%s err=%v", jobName, err)
		return nil
	}
	if !locked {
		log.Printf("【Worker】定时任务已由其他实例执行，跳过本实例 job=%s", jobName)
		return nil
	}
	defer func() {
		if _, err := lockStore.DelIfValue(key, owner); err != nil {
			log.Printf("【Worker】释放定时任务分布式锁失败 job=%s err=%v", jobName, err)
		}
	}()
	return job()
}
...
func registerAutoUpdateProductsJob(scheduler *scheduler.Scheduler, exchangeService *services.ExchangeService, configRepo *repository.SystemConfigRepository, accountRepo *repository.AccountRepository, lockStore workerJobLeaseStore) {
	// 每天凌晨3点更新商品
...
func registerAccountHealthCheckJob(scheduler *scheduler.Scheduler, tokenManager *services.TokenManager, accountRepo *repository.AccountRepository, notifier notification.Notifier, lockStore workerJobLeaseStore) {
	// 每6小时检查一次账号健康状态
...
// registerHistoryArchiveJob 注册历史日志/抢兑记录归档任务
func registerHistoryArchiveJob(scheduler *scheduler.Scheduler, archiveService *services.HistoryArchiveService, metrics *monitor.Metrics) {
	if archiveService == nil || !archiveService.Enabled() {
		log.Println("【历史归档】归档任务未启用，跳过注册")
		return
	}
	...
	_, err := scheduler.AddJobWithName(
		...
		func() error {
			if !running.CompareAndSwap(false, true) {
				...
				if metrics != nil {
					metrics.RecordHistoryArchiveRun("skipped", 0, 0, 0, false)
				}
				return nil
			}
			defer running.Store(false)
			...
			startedAt := time.Now()
			log.Println("【历史归档】开始执行历史归档任务...")
			result, err := archiveService.RunOnce(context.Background(), startedAt)
			if metrics != nil {
				status := "success"
				taskLogsMoved := int64(0)
				exchangeRecordsMoved := int64(0)
				hitBatchLimit := false
				if result != nil {
					taskLogsMoved = result.TaskLogsMoved
					exchangeRecordsMoved = result.ExchangeRecordsMoved
				...
				metrics.RecordHistoryArchiveRunAt(status, time.Now(), time.Since(startedAt), taskLogsMoved, exchangeRecordsMoved, hitBatchLimit)
			}
			return err
		},
