...
CREATE TABLE `exchange_tasks` (
    ...
    `scheduled_exchange_time` TIME NULL COMMENT ''任务级指定抢兑时间，为空则使用账号规则时间'',
    `restock_cycle` VARCHAR(20) DEFAULT ''daily'' COMMENT ''补货周期: daily/weekly/monthly/once'',
    `restock_weekday` TINYINT NULL COMMENT ''weekly 周期星期，0=周日'',
    `restock_day_of_month` TINYINT NULL COMMENT ''monthly 周期日期'',
    `restock_times` TEXT COMMENT ''补货时间点列表，逗号分隔 HH:MM[:SS]'',
    `custom_cron` VARCHAR(120) DEFAULT '''' COMMENT ''自定义 cron，优先于固定时间/补货时间'',
    `calendar_policy` VARCHAR(20) DEFAULT ''all'' COMMENT ''日历策略: all/workday/holiday'',
    `holiday_dates` TEXT COMMENT ''额外节假日 YYYY-MM-DD，逗号分隔'',
    `workday_dates` TEXT COMMENT ''调休工作日 YYYY-MM-DD，逗号分隔'',
    `skip_reason` VARCHAR(255) DEFAULT '''' COMMENT ''最近一次调度跳过原因'',
