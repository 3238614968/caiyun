...
CALL AddColumnIfMissing(
    'exchange_tasks',
    'active_dedupe_key',
    'CHAR(64) GENERATED ALWAYS AS (CASE WHEN `deleted_at` IS NULL AND `status` IN (''pending'', ''running'') THEN SHA2(CONCAT_WS(''|'', `user_id`, `exchange_rule_id`, `product_id`, COALESCE(NULLIF(`task_type`, ''''), ''fixed''), COALESCE(NULLIF(`scheduled_exchange_time`, ''''), ''rule_time''), COALESCE(NULLIF(`restock_cycle`, ''''), ''daily''), COALESCE(`restock_weekday`, -1), COALESCE(`restock_day_of_month`, -1), COALESCE(NULLIF(`restock_times`, ''''), ''-''), COALESCE(NULLIF(`custom_cron`, ''''), ''-''), COALESCE(NULLIF(`calendar_policy`, ''''), ''all''), COALESCE(NULLIF(`holiday_dates`, ''''), ''-''), COALESCE(NULLIF(`workday_dates`, ''''), ''-'')), 256) ELSE NULL END) VIRTUAL'
);

CALL CreateUniqueIndexIfMissing('exchange_tasks', 'uk_exchange_tasks_active_dedupe', '`active_dedupe_key`');
